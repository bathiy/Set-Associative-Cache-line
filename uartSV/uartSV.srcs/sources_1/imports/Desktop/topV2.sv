...
module FPGACacheSystem (
    ...
    logic        mem_valid;
    logic        mem_ready;
    logic        mem_wrEn;      // NEW: Write Enable
    logic [31:0] mem_wrData;    // NEW: Write Data
    ...
    FourWaySetAssociativeCache cache_inst (
        ...
        // Connect Write-Through Interface
        .io_mem_wrEn(mem_wrEn),
        .io_mem_wrData(mem_wrData),
    ...
    typedef enum logic [3:0] {
        ...
        RX_GET_WR_DATA,
        RX_GET_WR_MASK,
        DO_WRITE_REQ,    // <-- CRITICAL FIX: Holds write signal during Miss/Stall
        RX_GET_RAM_DATA,
        WAIT_FOR_CACHE, 
        SEND_RESPONSE   
    } state_t;
    ...
    always_ff @(posedge sys_clk) begin
        if (reset) begin
            ...
            case (state)
                IDLE: begin
                    // Safely drop signals ONLY if Cache is not stalled
                    if (!cpu_stall) begin
                        cpu_reqValid <= 0;
                        cpu_wrEn <= 0; 
                    end
                ...
                DO_WRITE_REQ: begin
                    ...
                    // If Cache Misses, stall=1. We stay here, holding signals High.
                    // When Refill finishes, stall=0. We go to IDLE.
                    if (!cpu_stall) begin
                        state <= IDLE;
                    end
                end
