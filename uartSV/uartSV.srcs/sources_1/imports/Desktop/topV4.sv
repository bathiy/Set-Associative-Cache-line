    ...
    always_ff @(posedge sys_clk) begin
        if (reset) begin
            ...
            case (state)
                ...
                // ------------------------------------------------
                IDLE: begin
                    // SAFETY: Drop signals only if cache is NOT busy.
                    // If we just finished a handshake (respValid), we can drop.
                    if (cpu_respValid || !cpu_stall) begin
                        cpu_reqValid <= 0;
                        cpu_wrEn <= 0; 
                    end
                ...
                WAIT_FOR_CACHE: begin
                    // Keep request valid until we get a RESPONSE (Hit or Refill Done)
                    cpu_reqValid <= 1; 
                ...
                // ------------------------------------------------
                // WRITE EXECUTION (Fixed for Write Miss)
                // ------------------------------------------------
                DO_WRITE_REQ: begin
                    ...
                    // 2. WAIT FOR COMPLETION (Handshake)
                    // We wait for 'cpu_respValid' just like a Read.
                    // - On HIT: respValid goes High immediately.
                    // - On MISS: Cache stalls, refills, retries, and THEN asserts respValid.
                    // By waiting for this signal, we ensure the write is NEVER dropped.
